...
# 이미지 경로
source_folder = r"C:\jsj\MMC\FirstExam\MiniProject\pytorch-CycleGAN-and-pix2pix\results\pix2pix_exp_rain2shine_8batch\test_latest\images"
...
for i in range(0, len(files), 3):
    ...
    # 마지막 그룹이 3개 미만이면 경고 출력 후 종료
    if len(group) < 3:
        print("마지막 그룹에 이미지가 부족합니다:", group)
        break

    # 첫 번째 fake 이미지와 세 번째 real 이미지 경로 설정
    fake_image_path = os.path.join(source_folder, group[0])  # 첫 번째 fake 이미지
    real_image_path = os.path.join(source_folder, group[2])  # 세 번째 real 이미지
